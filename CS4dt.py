# -*- coding: utf-8 -*-
"""
Created on Sat Nov 14 15:37:40 2015
...
# this script runs with a fixed dx value for 2 dt's
dt = np.array((0.125, 0.0625, 0.03125))
dt = list(dt)
...
for w in range(0,len(dt)):
    ...
    Aa_cen = D/dx**2 + u/(2*dx)
    Ab_cen = -2*D/dx**2
    Ac_cen = D/dx**2 - u/(2*dx)
    ...
    a1 = 3./8.
    a2 = 1./8.
    Aa_q = -u*(-1+a1-2*a2)/dx + D/dx**2
    Ab_q = -2*D/dx**2 - u*(1-a1+a2-a1)/dx
    Ac_q = D/dx**2 - u*a1/dx
    Ad_q = -u*a2/dx
...
figure
plt.loglog(dt, RMS_trap, '--' ,label='RMS Trap')
plt.loglog(dt, RMS_cen, 'g^-', label='RMS Central Diff')
plt.loglog(dt, RMS_up, 'bs-', label='RMS Upwind')
plt.loglog(dt, RMS_q, 'r*-', label='RMS QUICK')
plt.xlabel('log(dt)')
plt.ylabel('log(RMS)')
plt.title('log(RMS) vs dt')
plt.legend(loc='lower right', numpoints = 1)

# fit with np.polyfit
m_trap, b = np.polyfit(log10(dt), log10(RMS_trap), 1)
m_cen, b = np.polyfit(log10(dt), log10(RMS_cen), 1)
m_up, b = np.polyfit(log10(dt), log10(RMS_up), 1)
m_q, b = np.polyfit(log10(dt), log10(RMS_q), 1)
print 'slope'
print m_trap
print m_cen
print m_up
